# Project Configuration - Proamory
...
echo $PROAMORY_SUPABASE_URL      # Required
echo $PROAMORY_SUPABASE_ACCESS_TOKEN  # Required for MCP
```
...
| **Credential File** | `~/.env.credentials/hobby.env` (shared) + `proamory.env` (project-specific) |
...
### Verify Isolation
```bash
gh auth status  # Should show: not-configured
```
...
4. **Be explicit about repo**: `owner:not-configured repo:REPO_NAME`
...
```bash
# MCP (preferred)
Use mcp__github__list_issues for owner:not-configured repo:REPO_NAME
