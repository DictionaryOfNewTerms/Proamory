...
| **Token Variable** | `PROAMORY_GITHUB_TOKEN` |
| **Username Variable** | `PROAMORY_GITHUB_USERNAME` |
| **Credential File** | `~/.env.credentials/proamory.env` |
...
```bash
echo $PROAMORY_GITHUB_TOKEN  # Should show token (not NOT_CONFIGURED)
...
| Setting | Value |
|---------|-------|
| Token Variable | `PROAMORY_[SERVICE]_TOKEN` |
| Project ID | `xxx` |
...
vercel --prod --token $PROAMORY_VERCEL_TOKEN
```
